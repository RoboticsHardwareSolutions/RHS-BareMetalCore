...
int sdo_set_rx_pdo(CanOpenApp* app, CO_Data* d, PDOField pdo_field, NumPDOType pdo, uint8_t node_id, void* data)
{
    ...
    return can_open_write_sdo(app, d, node_id, field.index, field.subindex, field.size, data, NULL);
}
                       ...
                       uint8_t     size)
{
    ...
    return can_open_write_sdo(app, d, node_id, field.index + num_pdo, field.subindex, field.size, &map, NULL);
}
...
int sdo_set_rx_pdo_map_group(CanOpenApp* app, CO_Data* d, uint8_t node_id, NumPDOType num_pdo, PDOMapType num_map)
{
    ...
    return can_open_write_sdo(app, d, node_id, field.index, field.subindex, field.size, &num_map, NULL);
}
