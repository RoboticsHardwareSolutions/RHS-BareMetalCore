#pragma once

#include <stdint.h>
#include <stdbool.h>
#include "rhs_hal.h"

#include "tusb.h"

extern RHSHalUsbInterface usb_cdc_net_desc;

/**
 * @brief Virtual dispatch table for TinyUSB network callbacks.
 *
 * Register an instance with rhs_hal_cdc_net_set() to route the three
 * TinyUSB network callbacks to the currently active USB network backend
 * (usb_cdc_net or usb_eth_bridge).  Only one backend may be active at a time.
 */
typedef struct
{
    bool (*recv)(const uint8_t* buf, uint16_t len, void* context);
    void (*init)(void* context);
    uint16_t (*xmit)(uint8_t* dst, void* ref, uint16_t arg, void* context);
    void* context; /* optional user context pointer, not used by rhs_hal_cdc_net.c */
} TudNetOps;

/**
 * @brief Set the active TinyUSB network backend.
 *
 * Must be called before tusb_init() / after the previous backend is stopped.
 * @param ops  Pointer to a statically allocated ops table, or NULL to clear.
 */
void rhs_hal_cdc_net_set(const TudNetOps* ops);

/**
 * @brief Clear the active backend (equivalent to rhs_hal_cdc_net_set(NULL)).
 */
void rhs_hal_cdc_net_clear(void);
