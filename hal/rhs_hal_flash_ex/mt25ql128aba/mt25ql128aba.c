...
/**
  ...
  */
int32_t mt25ql128aba_ResetEnable(QSPI_HandleTypeDef *Ctx, mt25ql128aba_interface_t Mode) {
...
* @retval QSPI memory status
*/
int32_t mt25ql128aba_ResetMemory(QSPI_HandleTypeDef *Ctx, mt25ql128aba_interface_t Mode) {
    QSPI_CommandTypeDef s_command;
...
/**
  ...
  * @retval QSPI memory status
  */
int32_t mt25ql128aba_Enter4BytesAddressMode(QSPI_HandleTypeDef *Ctx, mt25ql128aba_interface_t Mode) {
    QSPI_CommandTypeDef s_command;
    ...
    else if (mt25ql128aba_auto_polling_mem_ready(Ctx, Mode) != 0) {
        return -1;
    }
...
/**
  ...
  * @retval BSP status
  */
static int32_t QSPI_DummyCyclesCfg(mt25ql128aba_interface_t Mode) {
    int32_t ret = HAL_OK;
    QSPI_CommandTypeDef s_command;
    uint16_t reg = 0;
    ...
    MODIFY_REG(reg, 0xF0F0, ((MT25QL128ABA_DUMMY_CYCLES_READ_QUAD << 4) |
                             (MT25QL128ABA_DUMMY_CYCLES_READ_QUAD << 12)));
...
/**
  ...
  * @retval QSPI memory status
  */
int32_t MT25QL128ABA_EnterQPIMode(QSPI_HandleTypeDef *Ctx)
{
    QSPI_CommandTypeDef s_command;
...
/**
  ...
  * @retval QSPI memory status
  */
int32_t MT25QL128ABA_ExitQPIMode(QSPI_HandleTypeDef *Ctx)
{
    QSPI_CommandTypeDef s_command;
...
int32_t QSPI_ConfigFlash(uint32_t Instance, mt25ql128aba_interface_t Mode, mt25ql128aba_transfer_t Rate) {
    int32_t ret = HAL_OK;

    /* Setup MCU transfer rate setting ***************************************************/
    hqspi.Init.SampleShifting = (Rate == MT25QL128ABA_STR_TRANSFER) ? QSPI_SAMPLE_SHIFTING_HALFCYCLE
...
int8_t mt25ql128aba_init(QSPI_HandleTypeDef *Ctx) {/* QSPI memory reset */
    ...
    else if (mt25ql128aba_auto_polling_mem_ready(&hqspi, MT25QL128ABA_QPI_MODE) != 0) {
        return -1;
...
int8_t
mt25ql128aba_read(QSPI_HandleTypeDef *Ctx, mt25ql128aba_interface_t Mode, uint8_t *pData, uint32_t ReadAddr,
                  uint32_t Size) {
    QSPI_CommandTypeDef s_command;
    switch (Mode) {
...
int8_t
mt25ql128aba_page_program(QSPI_HandleTypeDef *Ctx, mt25ql128aba_interface_t Mode, uint8_t *pData, uint32_t WriteAddr,
                          uint32_t Size) {
    QSPI_CommandTypeDef s_command;
    switch (Mode) {
...
int8_t mt25ql128aba_block_erase(QSPI_HandleTypeDef *Ctx, mt25ql128aba_interface_t Mode , uint32_t BlockAddress, mt25ql128aba_erase_t BlockSize)
{
    QSPI_CommandTypeDef s_command;
    switch(BlockSize)
    {
