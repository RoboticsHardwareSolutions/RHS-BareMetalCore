...
static bool stm32f_eth_tcpip_driver_init(struct mg_tcpip_if* ifp)
{
    struct stm32f_eth_driver_data* d        = (struct stm32f_eth_driver_data*) ifp->driver_data;
...
static size_t stm32f_eth_tcpip_driver_tx(const void* buf, size_t len, struct mg_tcpip_if* ifp)
{
...
static uint8_t mcast_addr[6] = {0x01, 0x00, 0x5e, 0x00, 0x00, 0xfb};

static void stm32f_eth_tcpip_driver_update_hash_table(struct mg_tcpip_if* ifp)
{
    // TODO(): read database, rebuild hash table
    ETH->MACA1LR = (uint32_t) mcast_addr[3] << 24 | (uint32_t) mcast_addr[2] << 16 | (uint32_t) mcast_addr[1] << 8 |
...
static bool stm32f_eth_tcpip_driver_poll(struct mg_tcpip_if* ifp, bool s1)
{
    if (ifp->update_mac_hash_table)
    {
        stm32f_eth_tcpip_driver_update_hash_table(ifp);
...
struct mg_tcpip_driver stm32f_eth_tcpip_driver = {stm32f_eth_tcpip_driver_init,
                                                  stm32f_eth_tcpip_driver_tx,
                                                  NULL,
                                                  stm32f_eth_tcpip_driver_poll};
