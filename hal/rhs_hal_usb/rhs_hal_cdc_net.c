...
void rhs_hal_cdc_net_set(const TudNetOps* ops)
{
...
void rhs_hal_cdc_net_clear(void)
{
    s_ops = NULL;
}
...
static const uint8_t* const configuration_fs_arr[CONFIG_ID_COUNT] =
    {[CONFIG_ID_RNDIS] = rndis_fs_configuration, [CONFIG_ID_ECM] = ecm_fs_configuration};

#    if TUD_OPT_HIGH_SPEED
static const uint8_t* const configuration_hs_arr[CONFIG_ID_COUNT] =
    {[CONFIG_ID_RNDIS] = rndis_hs_configuration, [CONFIG_ID_ECM] = ecm_hs_configuration};

// Size array for each configuration
static const uint16_t configuration_sz_arr[CONFIG_ID_COUNT] =
    {[CONFIG_ID_RNDIS] = MAIN_CONFIG_TOTAL_LEN, [CONFIG_ID_ECM] = ALT_CONFIG_TOTAL_LEN};
...
RHSHalUsbInterface usb_cdc_net_desc = {
...
// FIXME: usb_cdc_net_desc and usb_cdc_enet_desc are equal for reconfig. I want to stay only one of them
RHSHalUsbInterface usb_cdc_enet_desc = {
    .device_desc           = &desc_cdc_net,
    .configuration_arr     = configuration_fs_arr,  // TODO - support high speed configurations
    .string_desc_arr       = (char const* const*) string_desc_cdc_net_arr,
    .string_desc_arr_count = TU_ARRAY_SIZE(string_desc_cdc_net_arr),
