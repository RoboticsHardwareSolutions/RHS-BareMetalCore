...
static void hid_u2f_init(usbd_device* dev, RHSHalUsbInterface* intf, void* ctx);
...
static RHSSemaphore* hid_u2f_semaphore = NULL;
static bool hid_u2f_connected = false;

static HidU2fCallback callback;
static void* cb_ctx;

bool rhs_hal_hid_u2f_is_connected(void) {
    return hid_u2f_connected;
}

void rhs_hal_hid_u2f_set_callback(HidU2fCallback cb, void* ctx) {
    if(callback != NULL) {
...
RHSHalUsbInterface usb_hid_u2f = {
    .init = hid_u2f_init,
...
static void hid_u2f_init(usbd_device* dev, RHSHalUsbInterface* intf, void* ctx) {
    ...
    if(hid_u2f_semaphore == NULL) {
        hid_u2f_semaphore = rhs_semaphore_alloc(1, 1);
    }
    usb_dev = dev;
...
static void hid_u2f_on_suspend(usbd_device* dev) {
    ...
    if(hid_u2f_connected) {
        ...
        rhs_semaphore_release(hid_u2f_semaphore);
        if(callback != NULL) {
            callback(HidU2fDisconnected, cb_ctx);
        }
    }
...
void rhs_hal_hid_u2f_send_response(uint8_t* data, uint8_t len) {
    if((hid_u2f_semaphore == NULL) || (hid_u2f_connected == false)) return;
    rhs_assert(rhs_semaphore_acquire(hid_u2f_semaphore, RHSWaitForever) == RHSStatusOk);
    if(hid_u2f_connected == true) {
        usbd_ep_write(usb_dev, HID_EP_OUT, data, len);
    }
}

uint32_t rhs_hal_hid_u2f_get_request(uint8_t* data) {
